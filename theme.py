...
def _background_css():
    """Starfield as two GPU-composited overlay layers.

    The stars live on .stApp::before and ::after, which are fixed-position and
    animated with transform only. A transform moves an already-rasterized layer
    on the GPU — it never triggers a repaint, so scrolling stays smooth while
    the sky drifts. Animating background-position instead (the obvious approach)
    forces the browser to redraw every gradient on every frame.

    Each layer is oversized by one tile on each side and translated by exactly
    one tile, so the loop is seamless and no edge is ever exposed.
    """
    far = _starfield_layer(38, seed=11, max_radius=1.1, max_alpha=0.55)
    near = _starfield_layer(30, seed=22, max_radius=1.9, max_alpha=0.9)
...
/* Content must sit above the star layers, and must not paint its own
...
/* The review excerpt sits inside the card rather than in a hover overlay.
   An overlay has to out-stack Streamlit's own column containers, which it
   cannot reliably do — each column block makes its own stacking context, so
   the next card paints over it. In-card text always renders correctly.
   Full text is still available via the native title tooltip, which the
   browser draws in the top layer and never clips. */
.ach-quote {{
    margin-top: 12px;
    padding: 9px 12px;
    border-left: 2px solid {PLASMA_GREEN};
    background: rgba(55,230,160,0.06);
...
@media (prefers-reduced-motion: reduce) {{
    .stApp::before, .stApp::after, .halo, .halo-soft {{ animation: none !important; }}
