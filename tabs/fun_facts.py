...
def achievement_card(icon, title, value, subtitle="", review=""):
    # Escaped twice over: the excerpt is user-written text that may contain
    # quotes or angle brackets, and it lands both in body text and in a
    # title attribute.
    quote = ""
    if review:
        safe = html.escape(review, quote=True)
        quote = (f'<div class="ach-quote-label">From the review</div>'
                 f'<div class="ach-quote" title="{safe}">{safe}</div>')
    st.markdown(f"""
    <div class="ach-card">
...
def render(export_dir, metrics):
    ...
    st.caption(
        "Excerpts are trimmed previews of player-written reviews from the source dataset, shown for "
        "illustration. Hover an excerpt to see it in full."
    )
