...
def render(export_dir, metrics):
    ...
    with sub_clf:
        ...
        if cmx:
            ...
            z = [[tn, fp], [fn, tp]]
            labels = [["correctly flagged", "missed negatives"],
                      ["false alarms", "correctly flagged"]]
            text = [[f"<b>{z[r][c]:,}</b><br>{labels[r][c]}" for c in range(2)] for r in range(2)]

            figc = go.Figure(go.Heatmap(
                z=[[1, 0], [0, 1]],  # colour by correct/incorrect, not by magnitude
                ...
                colorscale=[[0, "rgba(255,107,91,0.22)"], [1, "rgba(55,230,160,0.18)"]],
                showscale=False, xgap=6, ygap=6,
                hovertemplate="%{y}<br>%{x}<extra></extra>",
            ))
            figc.update_layout(height=340, margin=dict(t=20, b=20, l=10, r=10))
            ...
            c3.metric("Errors in one direction", f"{fp / (fp + fn):.0%}",
                      help=f"{fp:,} of {fp + fn:,} total mistakes are 'said Recommended when it wasn't'")
